...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 2, 'input_size': (60, 256, 256), 'pool_size': None,
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=256, in_chans=60, num_classes=2, patch_size=16, embed_dim=96, depth=[2, 2, 6, 2],
                 split_size=[3, 5, 7],
...
def CSWin_96_24322_base_224(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=96, depth=[2, 4, 32, 2],
                             split_size=[64, 32, 16, 8], num_heads=[4, 8, 16, 32], mlp_ratio=4., **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model
...
if __name__ == '__main__':
    model = CSWin_96_24322_base_224().cuda()
    print(model)
    input_tensor = torch.randn(1, 60, 256, 256).cuda()
