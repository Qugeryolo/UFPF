...
from dataset.DGA_dataset_cls import HSIClassification, get_train_loader
...
parser.add_argument('--exp', type=str,
                    default='DGA/v1', help='experiment_name')
parser.add_argument('--snapshot_dir', type=str,
                    default='save_checkpoint', help='save model weight checkpoint')
parser.add_argument('--model', type=str,
                    default='v1', help='model_name')
...
train_dataset = HSIClassification(root=args.root_path, txt_name='fold3_train.txt', transforms=args.transforms,)

test_dataset = HSIClassification(root=args.root_path, txt_name='fold3_test.txt', transforms=args.transforms)
...
def train(args, snapshot_path):
    ...
    for epoch in range(args.nepochs):
        ...
        logging.info(
            'Epoch: %03d/%03d || total_Loss: %.4f '
            % (epoch + 1, args.nepochs, loss_total / len(train_loader)))

        if (epoch % 5 == 0) and (epoch >= 70) or (epoch == args.nepochs - 1):
            model.eval()
            ...
            logging.info(
                'Epoch: %03d/%03d || model1---Overall Accuracy: %.4f Average Accuracy: %.4f '
                'F1-score: %.4f Precision: %.4f Recall: %.4f Kappa: %.4f'
                % (epoch + 1, args.nepochs, metrics_avg[0], metrics_avg[1],
                   metrics_avg[2], metrics_avg[3], metrics_avg[4], metrics_avg[5]))
