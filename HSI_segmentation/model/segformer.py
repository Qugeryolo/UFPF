...
def build_segformer2d_model(config=None):
    model = SegFormer2d(
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            ...
            x_ = x.permute(0, 2, 1).reshape(B, C, n, n)
...
class MixVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, n, n, -1).permute(0, 3, 1, 2).contiguous()
        out.append(x)
...
class DWConv(nn.Module):
    ...
    def forward(self, x):
        ...
        n = cube_root(N)
        x = x.transpose(1, 2).view(B, C, n, n)
        x = self.dwconv(x)
        # added batchnorm (remove it ?)
        x = self.bn(x)
...
class SegFormerDecoderHead(nn.Module):
    ...
    def __init__(
        ...
        self.linear_fuse = nn.Sequential(
            ...
            nn.BatchNorm2d(decoder_head_embedding_dim),
            nn.ReLU(),
        )
        self.dropout = nn.Dropout(dropout)
...
if __name__ == "__main__":
    ...
    segformer2d = SegFormer2d(in_channels=60, num_classes=2).to("cuda:0")
    output = segformer2d(input)
    print(output.shape)
