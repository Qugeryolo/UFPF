...
class ConvDropoutNormNonlin(nn.Module):
    ...
    def __init__(self, input_channels, output_channels,
        ...
        # self.conv = self.conv_op(input_channels, output_channels, kernel_size=3, stride=1, padding=1, dilation=1, bias=True)
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            ...
            # print(x.size())
            # print(skips[-(u + 1)].size())
            x = torch.cat((x, skips[-(u + 1)]), dim=1)
            x = self.conv_blocks_localization[u](x)
            seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x)))
        fea = self.seg_outputs[4](x)
...
def initialize_network(threeD=True):
    ...
    '''
    # print(len(default_dict["net_num_pool_op_kernel_sizes"]))   3D model
    ...
    '''
    network = Generic_UNet(default_dict2d["num_input_channels"], default_dict2d["base_num_features"], default_dict2d["num_classes"], len(default_dict2d["net_num_pool_op_kernel_sizes"]),
                                default_dict2d["conv_per_stage"], 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
                                dropout_op_kwargs,
                                net_nonlin, net_nonlin_kwargs, False, False, lambda x: x, InitWeights_He(
                                    1e-2),
                                default_dict2d["net_num_pool_op_kernel_sizes"], default_dict2d["net_conv_kernel_sizes"], False, True, True)

    return network


if __name__ == "__main__":
    net = initialize_network(threeD=False).cuda()
    # print(net)
    m = torch.randn(1, 60, 256, 256).cuda()
    output = net(m)
    print(output.size())